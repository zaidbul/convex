...
interface MultiSelectProps
   ...
   */
  options: ReadonlyArray<MultiSelectOption> | ReadonlyArray<MultiSelectGroup>;
  /**
  ...
  /**
   * If provided, renders the multi-select trigger as the provided element.
   * Uses Base UI render prop pattern for polymorphic rendering.
   * Optional.
   */
...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
  (
    ...
    const isGroupedOptions = React.useCallback(
      (
        opts: ReadonlyArray<MultiSelectOption> | ReadonlyArray<MultiSelectGroup>,
      ): opts is ReadonlyArray<MultiSelectGroup> => {
    ...
    const arraysEqual = React.useCallback((a: string[], b: string[]): boolean => {
      ...
      return sortedA.every((val: string, index: number) => val === sortedB[index]);
    }, []);
    ...
    const getOptionByValue = React.useCallback(
      (value: string): MultiSelectOption | undefined => {
        const foundOption = getAllOptions().find((optionItem) => optionItem.value === value);
        if (!foundOption && process.env.NODE_ENV === "development") {
