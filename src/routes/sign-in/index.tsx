...
function SignInPage() {
  ...
  const form = useForm({
    ...
    onSubmit: async (data) => {
      ...
      if (step === "password") {
        ...
        if (!password) {
          setClerkError("Please enter your password")
          return
        }
        ...
        try {
          ...
          if (error) {
            setClerkError(
              error.longMessage || error.message || "Invalid email or password."
            )
            return
          }
      ...
      if (step === "forgotPassword") {
        if (!resetCode || resetCode.length < 6) {
          setClerkError("Please enter the 6-digit code")
          return
        }
  ...
  const handleForgotPassword = async () => {
    ...
    if (!email) {
      setClerkError("Please enter your email address first")
      return
    }
    ...
    setClerkError(null)
    try {
      // First create the sign-in with the identifier
